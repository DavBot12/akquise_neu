  ...
  private async scrapeCategory(category: string, options: ScrapingOptions): Promise<void> {
    ...
    try {
      ...
      for (let currentPage = 1; currentPage <= options.maxPages; currentPage++) {
        ...
        // Optimized selectors for maximum Willhaben listing detection
        const listingSelectors = [
          'article[data-testid*="search-result-entry"]',    // Primary Willhaben structure
          'div[data-testid*="search-result-entry"]',        // Alternative structure
          '[data-testid*="result"]:has(a[href*="/iad/"])',  // With property URLs
          'article:has(a[href*="/iad/"])',                  // Articles with property links
          '.MuiCard-root:has(a[href*="/iad/"])',           // Material UI cards
          '[data-cy*="result"]',                           // Cypress test selectors
          '.search-result-entry',                          // Legacy fallback
        ];
        
        let listings: any[] = [];
        let usedSelector = '';
        
        // Try multiple selectors and pick the one with most results
        let bestResults: { listings: any[], selector: string, count: number } = { listings: [], selector: '', count: 0 };
        
        for (const selector of listingSelectors) {
          try {
            const found = await page.$$(selector);
            if (found.length > bestResults.count) {
              bestResults = { listings: found, selector, count: found.length };
            }
          } catch (error) {
        ...
        listings = bestResults.listings;
        usedSelector = bestResults.selector;
        
        if (listings.length === 0) {
          options.onProgress(`[WARN] Keine Listings auf Seite ${currentPage} gefunden - versuche alternative Methoden`);
          
          // Alternative: Try finding any containers with property links
          const propertyLinks = await page.$$('a[href*="/iad/"]');
          if (propertyLinks.length > 0) {
            // Find parent containers of property links
            const containers: any[] = [];
            for (const link of propertyLinks) {
              const container = await link.evaluateHandle((el: Element) => {
                let parent = el.closest('article') || el.closest('div[class*="result"]') || el.closest('div[data-testid]');
                return parent || el.parentElement;
              });
              containers.push(container);
            }
            listings = containers;
            options.onProgress(`[INFO] Fallback: ${listings.length} Container über Property-Links gefunden`);
          }
        }
        
        options.onProgress(`[INFO] Gefunden mit "${usedSelector}": ${listings.length} Listings auf Seite ${currentPage}`);
